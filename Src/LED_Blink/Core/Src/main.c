...
int main(void)
{
  ...
  /* USER CODE BEGIN 1 */
	// Make sure exceptions use this application's vector table.
  SCB->VTOR = FLASH_BASE;
  /* USER CODE END 1 */
  ...
  while (1)
  {
		HAL_GPIO_WritePin(GPIOH, LED_B_Pin, GPIO_PIN_SET);		// Set GPIO state to high.
    HAL_GPIO_WritePin(GPIOH, LED_R_Pin, GPIO_PIN_RESET);	// Set GPIO state to low.
...
void SystemClock_Config(void)
{
  ...
  */
  /* This project assumes a real 25 MHz HSE source on PH0/PH1. */
